...
class GridEnvironment:
    ...
    def update_state_lid(self, current_state, action):
    ...
    def update_state(self, current_state, action):
        ...
        dist_to_goal_prev = np.sqrt((x - self.goal[0])**2 + (y - self.goal[1])**2)
        dist_to_goal_new = np.sqrt((new_x - self.goal[0])**2 + (new_y - self.goal[1])**2)

        dist_to_goal = dist_to_goal_prev - dist_to_goal_new
        reward = dist_to_goal

        # print("------------")
        # print(x, y, new_x, new_y)
        # print(dist_to_goal_prev, dist_to_goal_new)
        # print("dist_to_goal", dist_to_goal/ 10)
        ...
        if [int(new_x), int(new_y)] == self.goal:
            reward = 10
            done = True
        ...
        return [int(new_x), int(new_y), new_yaw], reward, done
    ...
    def plot_state_history_with_lidar(self):
